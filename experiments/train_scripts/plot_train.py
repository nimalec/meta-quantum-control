...
def plot_panel_b_support_vs_query(ax, data: Dict):
    ...
    iterations = data['val_iteration']
    support_loss = data["val_post_adapt"] 
    query_loss = data["val_pre_adapt"]
    ...
    ax.plot(smooth_iters, support_smooth, linewidth=2, color='orange', label='Support (post-adapt)')
    ax.plot(smooth_iters, query_smooth, linewidth=2, color='green', label='Query (pre-adapt)')
...
def plot_panel_d_stability(ax, data: Dict):
    ...
    ax.legend(lines1 , labels1 , fontsize=10, loc='upper right')
